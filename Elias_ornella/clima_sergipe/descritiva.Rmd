...
```{r echo=FALSE}
var <- sergipe %>% 
  dplyr::select(height, prcp, radiacao, umidade, press_atm,
         vento_velo, temp)
...
### Análise temporal multivariada

Gráficos _cross-correlation_ entre temperatura e demais covariáveis. Variáveis selecionadas: precipitação, umidade e velocidade do vento.
...
Dados faltantes nas covariáveis precipitação, umidade e velocidade do vento. 
...
Inputação de dados por interpolação linear.
...
Séries temporais das variáveis aparentemente estacionárias.
...
_*p: número de observações de defasagem incluídas no modelo, também conhecido como ordem de defasagem_
_*d: grau de diferenciação_
_*q: ordem da média móvel_
...
Ajuste do modelo VARMA(p=0,q=2).

```{r}
model_best <- VARMA(dados_diff, p = 0, q = 2)
...
```{r}
tsdisplay(residuals(model_best))
```

Predições.

```{r}
predictions <- VARMApred(model_best, h = 20)
```
